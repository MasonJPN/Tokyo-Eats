...
<>
  ...
  <p className="text-gray-400">
    ...
    <div className="min-h-screen bg-gray-950 flex justify-center items-start py-20 px-4">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-10 w-full max-w-lg text-gray-300">
        
        <h2 className="text-3xl font-semibold text-white mb-8 text-center">Add a Restaurant</h2>
        ...
        <form  onSubmit={handleSubmit} className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-3">
              ...
              {searchMsg ? <button onClick={handleSearch} type="button" className="bg-green-700 border border-green-700 text-white px-5 py-3 rounded-lg">
                Complete
              </button> :<button onClick={handleSearch} type="button" className="bg-gray-800 border border-gray-700 hover:border-gray-500 transition-colors text-white px-5 py-3 rounded-lg">
                Search
              </button>}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2 text-3xl">
              {[1,2,3,4,5].map((star) => (
                <span onClick={() => handleStarCount(star)}  key={star} 
                 className={star <= starCount ? " text-yellow-400  cursor-pointer": "text-gray-600"}>★</span>
              ))}
            </div>
          </div>

          <div className="flex items-center gap-3">
            <input  required type="checkbox" id="ate" className="w-4 h-4 accent-green-500" 
                onClick={handleAte}
            />
            <label htmlFor="ate" className="text-sm text-gray-400">I have visited this restaurant</label>
          </div>
